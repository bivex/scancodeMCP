...
server.registerTool("mcp_ScancodeMCP_analyze_license_file", {
...
server.registerTool("mcp_ScancodeMCP_summarize_license_risks", {
    ...
    for (const category in licenseData.problematic_licenses) {
        for (const item of licenseData.problematic_licenses[category]) {
            if (!riskMap[item.name])
                riskMap[item.name] = new Set();
...
server.registerTool("mcp_ScancodeMCP_compare_license_compatibility", {
...
server.registerTool("mcp_ScancodeMCP_list_high_risk_files", {
    ...
    for (const cat of highRiskCats) {
        if (!licenseData.problematic_licenses[cat])
            continue;
...
server.registerTool("mcp_ScancodeMCP_get_license_clause_summary", {
...
// --- LEGAL SUMMARY LOGIC ---
async function legalSummaryForLicense(licenseName, short = false) {
...
function licenseCompatibilityVerdict(licenseA, licenseB) {
    ...
    if (a === b)
        return `Both are ${licenseA}. Compatible.`;
